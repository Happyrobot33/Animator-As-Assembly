    ...
    public class AnimatorAsAssembly : MonoBehaviour
    {
        ...
        public AacFlState[,] ParseInstructions(string raw, AacFlLayer FX, out Register[] registers)
        {
            ...
            for (int i = 0; i < instructions.Length; i++)
            {
                ...
                {
                    case "INC":
                        Instructions = CopyIntoArray(
                            Instructions,
                            INC(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "DEC":
                        Instructions = CopyIntoArray(
                            Instructions,
                            DEC(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "JMP":
                    ...
                    case "ADD":
                        Instructions = CopyIntoArray(
                            ...
                            ADD(
                                Register.FindRegisterInArray(instructionParts[1], Registers),
                                Register.FindRegisterInArray(instructionParts[2], Registers),
                                Register.FindRegisterInArray(instructionParts[3], Registers),
                                FX
                            ),
                            i
                        );
                    ...
                    case "PUT":
                        Instructions = CopyIntoArray(
                            Instructions,
                            PUT(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "POP":
                        Instructions = CopyIntoArray(
                            Instructions,
                            POP(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "JSR":
                    ...
                    case "HALVE":
                        Instructions = CopyIntoArray(
                            Instructions,
                            HALF(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "SHL":
                        Instructions = CopyIntoArray(
                            Instructions,
                            SHL(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "SHR":
                        Instructions = CopyIntoArray(
                            Instructions,
                            SHR(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "BOOLTOINT":
                    ...
                    case "DRAWCHAR":
                        Instructions = CopyIntoArray(
                            Instructions,
                            DRAWCHAR(instructionParts[1], FX),
                            i
                        );
                        break;
                    case "WRITECHAR":
                        Instructions = CopyIntoArray(
                            Instructions,
                            WRITECHAR(instructionParts[1], FX),
                            i
                        );
                        break;
                    case "DELAY":
                        Instructions = CopyIntoArray(
                            Instructions,
                            DELAY(int.Parse(instructionParts[1]), FX),
                            i
                        );
                        break;
                    case "SHIFTSCREENRIGHT":
                        Instructions = CopyIntoArray(
                            Instructions,
                            SHIFTSCREENRIGHT(int.Parse(instructionParts[1]), FX),
                            i
                        );
                        break;
                    case "SHIFTSCREENDOWN":
                        Instructions = CopyIntoArray(
                            Instructions,
                            SHIFTSCREENDOWN(int.Parse(instructionParts[1]), FX),
                            i
                        );
                        break;
                    case "GETDIGIT":
                    ...
                    case "SHIFTLINERIGHT":
                        Instructions = CopyIntoArray(
                            Instructions,
                            SHIFTLINERIGHT(int.Parse(instructionParts[1]), FX),
                            i
                        );
                        break;
                    case "CLEARSCREEN":
                    ...
                    case "DRAWSTRING":
                        Instructions = CopyIntoArray(
                            Instructions,
                            DRAWSTRING(instructionParts[1], FX),
                            i
                        );
                        break;
                    case "INTTOBINARY":
                    ...
                    case "RAND8":
                        Instructions = CopyIntoArray(
                            Instructions,
                            RAND8(Register.FindRegisterInArray(instructionParts[1], Registers), FX),
                            i
                        );
                        break;
                    case "RANDOM":
        ...
        public AacFlState[] SWAP(Register register1, Register register2, AacFlLayer FX)
        {
            ...
            var movToSWAP = FX.NewState("{SWAP} MOV TO SWAP")
                .DrivingCopies(register1.param, SWAP1.param)
                .DrivingCopies(register2.param, SWAP2.param);
            var movToRegisters = FX.NewState("{SWAP} MOV TO REGISTERS")
                .DrivingCopies(SWAP1.param, register2.param)
                .DrivingCopies(SWAP2.param, register1.param);
        ...
        )
        {
            ...
            for (int i = calcStates.Length - 1; i > -1; i--)
            {
                ...
                if (DECREASING == 0)
                {
                    calcStates[i] = FX.NewState("{" + OPCODENAME + "} CALC " + power + "x")
                        .DrivingIncreases(IN1.param, power)
                        .DrivingDecreases(IN2.param, power);
                }
                else
                {
                    calcStates[i] = FX.NewState("{" + OPCODENAME + "} CALC " + power + "x")
                        .DrivingDecreases(IN1.param, power)
                        .DrivingDecreases(IN2.param, power);
                }
        ...
        )
        {
            ...
            var movToAAC = FX.NewState("{ADD} MOV TO AAC")
                .DrivingCopies(register1.param, AAC1.param)
                .DrivingCopies(register2.param, AAC2.param);
            var CalculationBranch = FX.NewState("{ADD} CALC BRANCH");

            AacFlState[] calc = GenerateCalculationBranch(
                0,
                AAC1,
                AAC2,
                "ADD",
            ...
            var movToRegister = FX.NewState("{ADD} MOV TO REGISTER")
                .DrivingCopies(AAC1.param, register3.param);
            ...
            AacFlTransition JIGRCheckTrue = JIGRCheck[JIGRCheck.Length - 1].TransitionsTo(
                swapAAC[0]
            );
            JIGRCheckTrue
                .When(JIGR.IsEqualTo(false))
                .And(AAC1.param.IsGreaterThan(0))
                .Or()
                .When(AAC2.param.IsLessThan(0))
                .And(AAC1.param.IsGreaterThan(0));
        ...
        )
        {
            ...
            var movToSAC = FX.NewState("{SUB} MOV TO SAC")
                .DrivingCopies(register1.param, SAC1.param)
                .DrivingCopies(register2.param, SAC2.param);
            ...
            var movToRegister = FX.NewState("{SUB} MOV TO REGISTER")
                .DrivingCopies(SAC1.param, register3.param);
        ...
        public AacFlState[] JEQ(Register register1, Register register2, AacFlLayer FX)
        {
            ...
            var movToJEQ = FX.NewState("{JEQ} MOV TO JEQ")
                .DrivingCopies(register1.param, JEQ1.param)
                .DrivingCopies(register2.param, JEQ2.param)
                .Drives(JEQR, false); //resets the flag to false
            var CalculationBranch = FX.NewState("{JEQ} CALC BRANCH");
            var Calculation1x = FX.NewState("{JEQ} CALC")
                .DrivingDecreases(JEQ1.param, 1)
                .DrivingDecreases(JEQ2.param, 1);
            var Calculation10x = FX.NewState("{JEQ} CALC")
                .DrivingDecreases(JEQ1.param, 10)
                .DrivingDecreases(JEQ2.param, 10);
            var Calculation100x = FX.NewState("{JEQ} CALC")
                .DrivingDecreases(JEQ1.param, 100)
                .DrivingDecreases(JEQ2.param, 100);
            var Calculation1000x = FX.NewState("{JEQ} CALC")
                .DrivingDecreases(JEQ1.param, 1000)
                .DrivingDecreases(JEQ2.param, 1000);
            ...
            movToJEQ1000x
                .When(JEQ1.param.IsGreaterThan(1000))
                .Or()
                .When(JEQ2.param.IsGreaterThan(1000));
            AacFlTransition movToJEQ100x = CalculationBranch.TransitionsTo(Calculation100x);
            movToJEQ100x
                .When(JEQ1.param.IsGreaterThan(100))
                .Or()
                .When(JEQ2.param.IsGreaterThan(100));
        ...
        public AacFlState[] JIG(Register register1, Register register2, AacFlLayer FX)
        {
            ...
            var movToJIG = FX.NewState("{JIG} MOV TO JIG")
                .DrivingCopies(register1.param, JIG1.param)
                .DrivingCopies(register2.param, JIG2.param)
                .Drives(JIGR, false); //resets the flag to false
            var CalculationBranch = FX.NewState("{JIG} CALC BRANCH");
            var Calculation1x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 1)
                .DrivingDecreases(JIG2.param, 1);
            var Calculation10x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 10)
                .DrivingDecreases(JIG2.param, 10);
            var Calculation100x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 100)
                .DrivingDecreases(JIG2.param, 100);
            var Calculation1000x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 1000)
                .DrivingDecreases(JIG2.param, 1000);
            var Calculation10000x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 10000)
                .DrivingDecreases(JIG2.param, 10000);
            var Calculation100000x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 100000)
                .DrivingDecreases(JIG2.param, 100000);
            var Calculation1000000x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 1000000)
                .DrivingDecreases(JIG2.param, 1000000);
            var Calculation10000000x = FX.NewState("{JIG} CALC")
                .DrivingDecreases(JIG1.param, 10000000)
                .DrivingDecreases(JIG2.param, 10000000);
            ...
            AacFlTransition movToJIG10000000x = CalculationBranch.TransitionsTo(
                Calculation10000000x
            );
            movToJIG10000000x
                .When(JIG1.param.IsGreaterThan(10000000))
                .Or()
                .When(JIG2.param.IsGreaterThan(10000000));
            AacFlTransition movToJIG1000000x = CalculationBranch.TransitionsTo(Calculation1000000x);
            movToJIG1000000x
                .When(JIG1.param.IsGreaterThan(1000000))
                .Or()
                .When(JIG2.param.IsGreaterThan(1000000));
            AacFlTransition movToJIG100000x = CalculationBranch.TransitionsTo(Calculation100000x);
            movToJIG100000x
                .When(JIG1.param.IsGreaterThan(100000))
                .Or()
                .When(JIG2.param.IsGreaterThan(100000));
            AacFlTransition movToJIG10000x = CalculationBranch.TransitionsTo(Calculation10000x);
            movToJIG10000x
                .When(JIG1.param.IsGreaterThan(10000))
                .Or()
                .When(JIG2.param.IsGreaterThan(10000));
            AacFlTransition movToJIG1000x = CalculationBranch.TransitionsTo(Calculation1000x);
            movToJIG1000x
                .When(JIG1.param.IsGreaterThan(1000))
                .Or()
                .When(JIG2.param.IsGreaterThan(1000));
            AacFlTransition movToJIG100x = CalculationBranch.TransitionsTo(Calculation100x);
            movToJIG100x
                .When(JIG1.param.IsGreaterThan(100))
                .Or()
                .When(JIG2.param.IsGreaterThan(100));
        ...
        )
        {
            ...
            var movToMAC = FX.NewState("{MUL} MOV TO MAC")
                .DrivingCopies(register2.param, MAC2.param)
                .DrivingCopies(register1.param, MAC3.param)
                .Drives(MAC4.param, 0)
                .Drives(MAC1.param, 0);
            ...
            var movToRegister = FX.NewState("{MUL} MOV TO REGISTER")
                .DrivingCopies(MAC1.param, register3.param);
            ...
            AacFlTransition CalcNotDone = Condition[Condition.Length - 1].TransitionsTo(
                Calculation[0]
            );
        ...
        public AacFlState[] MULN(Register rin, int value, Register rout, AacFlLayer FX)
        {
            ...
            AacFlState staticMultiply = FX.NewState("{MULN} MULTIPLY")
                .DrivingRemaps(
                    rin.param,
                    0,
                    ((int.MaxValue - 1) / value),
                    rout.param,
                    0,
                    int.MaxValue - 1
                );
        ...
        )
        {
            ...
            var movToDAC = FX.NewState("{DIV} MOV TO DAC")
                .DrivingCopies(register1.param, DAC1.param)
                .DrivingCopies(register2.param, DAC2.param)
                .Drives(DAC3.param, 0);
            ...
            var OneDivideRemainderFix = FX.NewState("{DIV} ONE DIVIDE REMAINDER FIX")
                .Drives(DAC2.param, 1)
                .Drives(DAC3.param, 0);
            ...
            //move the result to the register
            var movToRegister = FX.NewState("{DIV} MOV TO REGISTER")
                .DrivingCopies(DAC3.param, register3.param);

            //transitions
            AacFlTransition OneDivideRemainderFixTransition = movToDAC.TransitionsTo(
                OneDivideRemainderFix
            );
        ...
        public AacFlState[] PUT(Register register, AacFlLayer FX)
        {
            ...
            var incrementStackPointer = FX.NewState("{PUT} INCREMENT STACK POINTER")
                .DrivingIncreases(stackPointer.param, 1);
            AacFlState[] stackEntrys = new AacFlState[stackSize];
            for (int i = 0; i < stackSize; i++)
            {
                var movToStack = FX.NewState("{PUT} MOV TO STACK")
                    .DrivingCopies(register.param, stack[i].param);
        ...
        public AacFlState[] POP(Register register, AacFlLayer FX)
        {
            ...
            var entranceState = FX.NewState("{POP} STACK BRANCH")
                .DrivingDecreases(stackPointer.param, 1);
            var branchMerge = FX.NewState("{POP} BRANCH MERGE");
            AacFlState[] stackEntrys = new AacFlState[stackSize];
            for (int i = 0; i < stackSize; i++)
            {
                var movToRegister = FX.NewState("{POP} MOV TO REGISTER")
                    .DrivingCopies(stack[i].param, register.param);
        ...
        public AacFlState[] DOUBLE(Register register, AacFlLayer FX)
        {
            ...
            var doubleState = FX.NewState("{DOUBLE} DOUBLE REGISTER")
                .DrivingRemaps(
                    register.param,
                    0,
                    (int.MaxValue / 2) - 1,
                    doubleRegister.param,
                    0,
                    int.MaxValue - 1
                );
            var move = FX.NewState("{DOUBLE} MOVE REGISTER")
                .DrivingCopies(doubleRegister.param, register.param);
        ...
        public AacFlState[] HALF(Register register, AacFlLayer FX)
        {
            ...
            var halfState = FX.NewState("{HALF} HALF REGISTER")
                .DrivingRemaps(
                    register.param,
                    0,
                    int.MaxValue - 1,
            ...
            var move = FX.NewState("{HALF} MOVE REGISTER")
                .DrivingCopies(halfRegister.param, register.param);
        ...
        public AacFlState[] SHL(Register register, AacFlLayer FX)
        {
            ...
            var shlState = FX.NewState("{SHL} SHL REGISTER")
                .DrivingRemaps(
                    register.param,
                    0,
                    (int.MaxValue / 10) - 1,
            ...
            var move = FX.NewState("{SHL} MOVE REGISTER")
                .DrivingCopies(shlRegister.param, register.param);
        ...
        public AacFlState[] SHR(Register register, AacFlLayer FX)
        {
            ...
            //10-99 becomes 1-9
            var shrState = FX.NewState("{SHR} SHR REGISTER")
                .DrivingRemaps(
                    register.param,
                    10,
                    int.MaxValue - 1,
            ...
            var move = FX.NewState("{SHR} MOVE REGISTER")
                .DrivingCopies(shrRegister.param, register.param);
        ...
        public AacFlState[] BOOLTOINT(Register contact, Register register, int value, AacFlLayer FX)
        {
            ...
            var driveIntIfContact = FX.NewState("{BOOLTOINT} DRIVE INT IF CONTACT")
                .Drives(register.param, value);
        ...
        public AacFlState[] SEGINT(Register rin, Register out_1, Register out_2, AacFlLayer FX)
        {
            ...
            var movToRegister = FX.NewState("{SEGMENTINT} MOV TO REGISTER")
                .DrivingCopies(rin.param, SI1.param)
                .DrivingCopies(rin.param, SI2.param);

            var numberNotLargeEnough = FX.NewState("{SEGMENTINT} NUMBER NOT LARGE ENOUGH")
                .DrivingCopies(SI1.param, out_2.param)
                .Drives(out_1.param, 0);
            ...
            var movToOut = FX.NewState("{SEGMENTINT} MOV TO OUT")
                .DrivingCopies(SI1.param, out_1.param)
                .DrivingCopies(SI2.param, out_2.param);
        ...
        public AacFlState[] DELAY(int frames, AacFlLayer FX)
        {
            ...
            var delay = FX.NewState("{DELAY} DELAY")
                .WithAnimation(aac.DummyClipLasting(seconds, AacFlUnit.Seconds));
            ...
            for (int t = 0; t < text.Length; t++)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    ...
                    catch (KeyNotFoundException)
                    {
                        ...
                        EditorUtility.ClearProgressBar();
                        throw new Exception(
                            "Character " + character + " not found in font provided"
                        );
                    }
                    int VRAMx = (i % 3) + (t * 4);
                    int VRAMy = i / 3;
        ...
        public AacFlState[] GETDIGIT(Register rIN, Register rOUT, int digit_place, AacFlLayer FX)
        {
            ...
            AacFlState movToRegister = FX.NewState("{GETDIGIT} MOV TO REGISTER")
                .DrivingRemaps(
                    rIN.param,
                    0,
                    int.MaxValue,
                    GETD.param,
                    0,
                    int.MaxValue / (int)Math.Pow(10, digit_place)
                )
                .DrivingRemaps(
                    rIN.param,
                    0,
            ...
            AacFlState movToOutput = FX.NewState("{GETDIGIT} MOV TO OUTPUT")
                .DrivingCopies(GETD.param, rOUT.param);
        ...
        public AacFlState[] DRAWCOMPLETEREGISTER(Register r, AacFlLayer FX)
        {
            ...
            AacFlState RESETREG = FX.NewState("{DRAWCOMPLETEREGISTER} RESETREG")
                .Drives(CURDIG.param, 0); //reset CURDIG to 0
        ...
        public AacFlState[] INTTOBINARY(Register rin, Register rout, AacFlLayer FX)
        {
            ...
            AacFlState movToRegister = FX.NewState("{INTTOBINARY} movToRegister")
                .DrivingCopies(rin.param, INTTOBINARYAC1.param)
                .Drives(INTTOBINARYACR.param, 0);
            ...
            AacFlState BitIsOne = FX.NewState("{INTTOBINARY} BitIsOne")
                .DrivingIncreases(INTTOBINARYACR.param, 3); //increase to 4 to represent a 1
            BitIsOne.AutomaticallyMovesTo(CalculationBranch);
            AacFlState movToOutput = FX.NewState("{INTTOBINARY} movToOutput")
                .DrivingCopies(INTTOBINARYACR.param, rout.param);
            ...
            AacFlState setOutputToZero = FX.NewState("{INTTOBINARY} setOutputToZero")
                .Drives(INTTOBINARYACR.param, 1);
            ...
            AacFlTransition CalculationBranchToShiftState = ShiftState[
                ShiftState.Length - 1
            ].TransitionsTo(BitIsOne);
        ...
        public AacFlState[] BINARYTOINT(Register rin, Register rout, AacFlLayer FX)
        {
            ...
            AacFlState BIT_PLACE_ONE = FX.NewState("{BINARYTOINT} BIT_PLACE_ONE")
                .DrivingIncreases(BINARYTOINTAC1.param, 1);
            //determine if get bit moves to BIT_PLACE_ONE if BINARYTOINTAC2 is 4
            AacFlTransition GetBitToBIT_PLACE_ONE = GetBit[GetBit.Length - 1].TransitionsTo(
                BIT_PLACE_ONE
            );
            ...
            AacFlState BIT_PLACE_TWO = FX.NewState("{BINARYTOINT} BIT_PLACE_TWO")
                .DrivingIncreases(BINARYTOINTAC1.param, 2);
            AacFlTransition GetBitToBIT_PLACE_TWO = GetBit2[GetBit2.Length - 1].TransitionsTo(
                BIT_PLACE_TWO
            );
            ...
            AacFlState BIT_PLACE_THREE = FX.NewState("{BINARYTOINT} BIT_PLACE_THREE")
                .DrivingIncreases(BINARYTOINTAC1.param, 4);
            AacFlTransition GetBitToBIT_PLACE_THREE = GetBit3[GetBit3.Length - 1].TransitionsTo(
                BIT_PLACE_THREE
            );
            ...
            AacFlState BIT_PLACE_FOUR = FX.NewState("{BINARYTOINT} BIT_PLACE_FOUR")
                .DrivingIncreases(BINARYTOINTAC1.param, 8);
            AacFlTransition GetBitToBIT_PLACE_FOUR = GetBit4[GetBit4.Length - 1].TransitionsTo(
                BIT_PLACE_FOUR
            );
            ...
            AacFlState BIT_PLACE_FIVE = FX.NewState("{BINARYTOINT} BIT_PLACE_FIVE")
                .DrivingIncreases(BINARYTOINTAC1.param, 16);
            AacFlTransition GetBitToBIT_PLACE_FIVE = GetBit5[GetBit5.Length - 1].TransitionsTo(
                BIT_PLACE_FIVE
            );
            ...
            AacFlState BIT_PLACE_SIX = FX.NewState("{BINARYTOINT} BIT_PLACE_SIX")
                .DrivingIncreases(BINARYTOINTAC1.param, 32);
            AacFlTransition GetBitToBIT_PLACE_SIX = GetBit6[GetBit6.Length - 1].TransitionsTo(
                BIT_PLACE_SIX
            );
            ...
            AacFlState BIT_PLACE_SEVEN = FX.NewState("{BINARYTOINT} BIT_PLACE_SEVEN")
                .DrivingIncreases(BINARYTOINTAC1.param, 64);
            AacFlTransition GetBitToBIT_PLACE_SEVEN = GetBit7[GetBit7.Length - 1].TransitionsTo(
                BIT_PLACE_SEVEN
            );
            ...
            AacFlState BIT_PLACE_EIGHT = FX.NewState("{BINARYTOINT} BIT_PLACE_EIGHT")
                .DrivingIncreases(BINARYTOINTAC1.param, 128);
            AacFlTransition GetBitToBIT_PLACE_EIGHT = GetBit8[GetBit8.Length - 1].TransitionsTo(
                BIT_PLACE_EIGHT
            );
            GetBitToBIT_PLACE_EIGHT.When(BINARYTOINTAC2.param.IsNotEqualTo(1));

            //final state
            AacFlState FINAL = FX.NewState("{BINARYTOINT} FINAL")
                .DrivingCopies(BINARYTOINTAC1.param, rout.param);
        ...
        public AacFlState[] RAND8(Register rout, AacFlLayer FX)
        {
            AacFlState randomize = FX.NewState("{RAND8} randomize")
                .DrivingRandomizes(rout.param, 0, 255);
        ...
        public AacFlState[] RANDOM(int min, int max, Register rout, AacFlLayer FX)
        {
            AacFlState randomize = FX.NewState("{RANDOM} randomize")
                .DrivingRandomizes(rout.param, min, max);
        ...
        public AacFlState[] DRAWCHARCODE(Register code, AacFlLayer FX)
        {
            ...
            for (int y = 0; y < 5; y++)
            {
                for (int x = 0; x < 3; x++)
                {
                    ...
                    DECISIONS[index] = FX.NewState("{DRAWCHARCODE} " + x + "," + y + " DECISION")
                        .Drives(ADDRESS, 0); //default to 0
                    ...
                    for (int i = 0; i < 255; i++)
                    {
                        ...
                        if (shouldDraw)
                        {
                            AacFlTransition PixelShouldBeOn = DECISIONS[index].TransitionsTo(
                                PIXELS[index]
                            );
        ...
        public AacFlState[] PIXEL(Register X, Register Y, AacFlLayer FX)
        {
            ...
            for (int i = 0; i < displayWidth; i++)
            {
                for (int j = 0; j < displayHeight; j++)
                {
                    ...
                    AacFlTransition entryToPixel = ENTRY.TransitionsTo(
                        PIXELS[i * displayHeight + j]
                    );
